...
module Drive {
  ...
  public func getFs(userFolders: FolderState, p: Principal) : [Text] {
    switch (Array.find<(Principal, [Text])>(userFolders, func(pair: (Principal, [Text])) { pair.0 == p })) {
      case (?(_, fs)) fs;
      case null        []
    }
  };
  ...
  public func putFs(
    ...
    let rebuilt = Array.foldLeft<(Principal,[Text]), FolderState>(
      userFolders,
      ([] : FolderState),
      func(acc: FolderState, pair: (Principal, [Text])): FolderState {
        if (pair.0 == p) Array.append(acc, [(p, fs)])
        else             Array.append(acc, [pair])
      }
    );
    if (Array.find<(Principal,[Text])>(userFolders, func(pr: (Principal, [Text])) { pr.0 == p }) == null) {
  ...
  public func createFolder(userFolders: FolderState, caller: Principal, name: Text) : FolderState {
    ...
    if (Array.find<Text>(fs, func(x: Text) { x == name }) == null) {
  ...
  public func deleteFolder(
    ...
    let newFs        = Array.filter<Text>(getFs(userFolders, caller), func(x: Text) { x != name });
    let updatedFold  = putFs(userFolders, caller, newFs);
    let updatedFiles = Array.map<FileMeta, FileMeta>(
      files,
      func(f: FileMeta): FileMeta {
  ...
  public func uploadFile(
    ...
    let fs = getFs(userFolders, caller);
    let updatedFolders = if (Array.find<Text>(fs, func(x: Text) { x == folder }) == null) {
  ...
  public func getUserFiles(files: FileState, caller: Principal) : [FileMeta] {
    Array.filter<FileMeta>(files, func(f: FileMeta) { f.owner == caller })
  };
};
